...
async def shop(update: Update, context: CallbackContext) -> None:
    ...
    reply_markup = _get_inline_keyboard(first_character)
...
async def next_character(update: Update, context: CallbackContext) -> None:
    ...
    if current_index + 1 < len(rarity_3_characters):
        current_index += 1
        next_character_data = rarity_3_characters[current_index]
        reply_markup = _get_inline_keyboard(next_character_data, current_index)

        caption = f"🪙Welcome to the Shop! Choose a character to buy:\n\n" \
                  f"🏮Anime Name: {next_character_data['anime']}\n" \
                  f"🎴Character Name: {next_character_data['name']}\n" \
                  f"🌀Rarity: {next_character_data['rarity']}\n" \
                  f"🎗️Character ID: {next_character_data['id']}\n" \
                  f"💸Coin Price: {next_character_data['coin_price']}"

        await context.bot.edit_message_media(
            chat_id=update.callback_query.message.chat_id,
            message_id=user_data['message_id'],
            media=InputMediaPhoto(media=next_character_data['img_url'], caption=caption),
            reply_markup=reply_markup
        )
...
def _get_inline_keyboard(character, current_index=0, total_count=7):
    keyboard = []
...
async def buy_character(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        await query.answer("Invalid character ID.")
        return
...
async def previous_character(update: Update, context: CallbackContext) -> None:
    ...
    if current_index - 1 >= 0:
        current_index -= 1
        previous_character_data = rarity_3_characters[current_index]
        reply_markup = _get_inline_keyboard(previous_character_data, current_index)

        caption = f"🪙Welcome back to the Shop! Choose a character to buy:\n\n" \
                  f"🏮Anime Name: {previous_character_data['anime']}\n" \
                  f"🎴Character Name: {previous_character_data['name']}\n" \
                  f"🌀Rarity: {previous_character_data['rarity']}\n" \
                  f"🎗️Character ID: {previous_character_data['id']}\n" \
                  f"💸Coin Price: {previous_character_data['coin_price']}"

        await context.bot.edit_message_media(
            chat_id=update.callback_query.message.chat_id,
            message_id=user_data['message_id'],
            media=InputMediaPhoto(media=previous_character_data['img_url'], caption=caption),
            reply_markup=reply_markup
        )
