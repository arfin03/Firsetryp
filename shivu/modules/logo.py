...
from Grabber import tbot, application 
...
async def lego(update, context):
    event = update.message

    # No need to check for owner ID, allowing all users to access the command
    ...
    pesan = await event.reply_text("`Logo in process. Please wait a moment...`")
    try:
        text = ' '.join(quew)  # joining arguments to form text
        ...
        await context.bot.send_photo(
            event.chat_id, photo=open(fname, 'rb'), caption="**Logo Generated by @dark_waifu_bot**"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception as e:
        await event.reply_text(f"An error occurred: {str(e)}")

async def vip_logo_handler(update, context):
    event = update.message
    quew = context.args
    ...
    img_url = quew[0]
    text = ' '.join(quew[1:])  # Joining arguments to form text
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            await event.reply_text("Failed to fetch the image.")
    except Exception as e:
        await event.reply_text(f"An error occurred: {str(e)}")
            
# Register the command handler
vip_logo_handler = CommandHandler("viplogo", vip_logo_handler)
application.add_handler(vip_logo_handler)


lego_handler = CommandHandler("logo", lego)
application.add_handler(lego_handler)
