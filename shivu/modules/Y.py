...
async def promote(update, context):
    ...
    await update.message.reply_text(f"{promoted_user.username} was promoted by {promoting_user.username}.")
...
async def demote(update, context):
    ...
    await update.message.reply_text(f"{demoted_user.username} was demoted by {demoting_user.username}.")

async def profile(update, context):
    query = update.callback_query
    await query.answer()
    await query.edit_message_text(text=f"Username: {query.data}")
